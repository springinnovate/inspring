...
def execute(args):
    ...
    task_graph = taskgraph.TaskGraph(work_token_dir, -1)
...
def _mask_raster_by_vector(
    ...
    geoprocessing.rasterize(vector_path, mask_raster_path, [1], None)
...
def _convert_landscape(
    ...
    lulc_raster_info = geoprocessing.get_raster_info(base_lulc_path)
    ...
    for step_index in range(n_steps):
        ...
        distance_nodata = geoprocessing.get_raster_info(
...
def _sort_to_disk(dataset_path, score_weight=1.0):
    ...
    dataset_info = geoprocessing.get_raster_info(dataset_path)
    ...
    for scores_data, scores_block in geoprocessing.iterblocks(
...
def _make_gaussian_kernel_path(sigma, kernel_path):
    ...
    for kernel_data, kernel_block in geoprocessing.iterblocks(
            (kernel_path, 1)):
